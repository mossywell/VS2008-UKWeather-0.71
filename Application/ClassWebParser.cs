...
namespace Mossywell.UKWeather
{
	internal class WebParser
	{
		...
		internal void GetAndParseURL(string url, string postcode)
		{
			...
			try
			{
				_strWorkingData = WebUtils.GetURI(url, Constants.DEFAULT_WEBPARSERTIMEOUT);
			...
			TrimLeftToThisString("Current Conditions");
			if(_strWorkingData == "")
			{
			...
			// Description
            strDesc = new Regex(@"<b>(.*?)</b>", RegexOptions.Singleline).Match(_strWorkingData).Groups[1].Value;
			if(strDesc == "")
			{
            ...
            _strTempCelsiusNow = new Regex(@"current_box_temp.*?(-?\d+)", RegexOptions.Singleline).Match(_strWorkingData).Groups[1].Value;
            ...
            strTempFLC = new Regex(@"Feels Like.*?(\d+)", RegexOptions.Singleline).Match(_strWorkingData).Groups[1].Value;
			if(strTempFLC == "") 
			{
